...
def load_train_validate_data_2(csv_file, root_dir, batch_size, valid_size=100, extra = True):
    ...
    test_data_set = DatasetTorch(csv_file=csv_file, root_dir=root_dir, transform=test_transforms)
    train_data_set = DatasetTorch(csv_file=csv_file, root_dir=root_dir, transform=train_transforms)
    ...
    # train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True)
    train_loader = torch.utils.data.DataLoader(train_data_set, sampler=train_sampler, batch_size=batch_size)
    val_loader = torch.utils.data.DataLoader(test_data_set, sampler=test_sampler, batch_size=batch_size)
    return train_loader, val_loader
...
def pytorch_cnn_train(model, num_epochs=1, model_file=None):
    ...
    optimizer = optim.Adam(model.classifier.parameters(), lr=learning_rate, betas = [0.9,0.999])
    ...
    for epoch in range(num_epochs):  # loop over the dataset multiple times
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            if (i + 1) % print_every == 0:
                ...
                print('Epoch [{}/{}], Step [{}/{}], Train loss: {:.4f}, '
                      'Test loss: {:.4f}, Test accuracy: {:.3f}, Duration: {}'.format(epoch+1, num_epochs,
        ...
        PATH = 'Densenet_161_{}_e.pth'.format(epoch)
